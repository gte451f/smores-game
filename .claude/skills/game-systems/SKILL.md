...
| [`topics/game-data.md`](topics/game-data.md) | The definition/record/actor storage model, `USmoresDefinition` and the three fields every definition shares, why presentation stays on the subclasses, id-vs-asset-pointer references, `USmoresDefinitionLibrary` look-up, Asset Manager registration, the two-layer content sweeps, and character records: `UCharacterDefinition`, `FCharacterRecord`, the GameState's record store, and the soft split between a record and its unit. **Read before adding a definition type or changing how authored data is stored.** |
...
| [`topics/factions.md`](topics/factions.md) | `UFactionDefinition`, the per-session `UWorldFactionComponent` (faction records with current tier, the symmetric faction↔faction standing matrix) on the GameState, the per-player `UPlayerStandingComponent` on the PlayerState, the -100..100 standing scale, and the rule that nothing reads standing for behavior yet |
| [`topics/dialog.md`](topics/dialog.md) | Dialog loaded from plain text at startup (the base game as package #0, mods beside it), `mod.json` and `Requires` ordering, the bark CSV format, the condition language and the registered facts, "most specific wins" bark selection, the server-side `UBarkDirectorComponent`, delivery to the feed as line ids, runtime string tables and the localized text source, the dialog execs. **The writers' and modders' reference lives here too.** |
| [`topics/hud-and-panels.md`](topics/hud-and-panels.md) | The always-on HUD frame — its six anchored regions, the nav rail and the panel windows it opens, the resource strip that holds gold, and the rule that stops a click on the HUD from also ordering the squad somewhere. **Read before adding anything to the HUD or any new panel.** |
