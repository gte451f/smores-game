...
| [`topics/inventory.md`](topics/inventory.md) | `UItemDefinition` shared item-type assets vs. `FInventoryItem` carried instances, `UInventoryComponent` fixed-slot item storage, `UInventoryWidget` drag-and-drop display, PlayerController-managed open/close/toggle for pawn/container/loot |
