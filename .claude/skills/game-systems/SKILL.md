...
| [`topics/game-data.md`](topics/game-data.md) | The definition/record/actor storage model, `USmoresDefinition` and the three fields every definition shares, why presentation stays on the subclasses, id-vs-asset-pointer references, `USmoresDefinitionLibrary` look-up, Asset Manager registration, and the two-layer content sweeps. **Read before adding a definition type or changing how authored data is stored.** |
