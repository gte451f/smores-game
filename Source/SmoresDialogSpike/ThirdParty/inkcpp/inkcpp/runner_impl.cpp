		...
		if (cmd >= Command::OP_BEGIN && cmd < Command::OP_END) {
			...
			switch (cmd) {
				...
				case Command::CHOICE: {
					...
					// smores fix (dialog spike, 2026-09-24): decide whether the choice is shown, but
					// always consume what evaluating it left behind - its condition, its text and its
					// tags - as the reference ink runtime does. Skipping with a bare `break` left the
					// text on the eval stack and the tags pending, so the next choice that *was* shown
					// inherited a hidden choice's tags (its #id: came out as the hidden one's).
					...
					if (! show_choice) {
						// An empty list files the pending tags nowhere, which drops them.
						assign_tags({});
						break;
					}
